...
class BlockTrade(unittest.TestCase):
    ...
    股转\证券转换\缩股
    """
    yaml = BaseAction().read_yaml(path=PathConfig().share_reconciliation())['hebing']
    ...
    def test_block_trade(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：股转\证券转换\缩股 准备数据')
        ...
        if not sql_result:
            logger().info('股转\证券转换\缩股 准备数据完成')
            assert True
        else:
            logger().error('股转\证券转换\缩股 准备数据异常')
