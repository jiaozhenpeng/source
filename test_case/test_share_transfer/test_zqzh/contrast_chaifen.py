...
class ContrastEtfSplit(unittest.TestCase):
    ...
    股转\证券转换\扩股
    """
    yaml = BaseAction().read_yaml(path=PathConfig().share_reconciliation())['chaifen']
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：股转\证券转换\扩股 对比数据')
        ...
        if not final_result :
            logger().info('股转\证券转换\扩股 对比数据无异常')
            assert True
        else:
            logger().error('股转\证券转换\扩股 对比数据异常')
