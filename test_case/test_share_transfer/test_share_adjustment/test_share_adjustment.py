...
class ShareAdjustment(unittest.TestCase):
    """
    股转\股份调账
    """
    yaml = BaseAction().read_yaml(PathConfig().share_reconciliation())['ShareAdjustment']
    def test_adjustment(self):
        ...
        if not sql_result:
            logger().info('股转\股份调账 准备数据完成')
            assert True
        else:
            logger().error('股转\股份调账 准备数据异常')
