...
class ContrastRestrictedShares(unittest.TestCase):
    ...
    股转\DVP改革\违约处置（T+2）
    """
    yaml = BaseAction().read_yaml(path=PathConfig().share_reconciliation())['DVP']['Tday2']
    ...
    def test_restricted_shares(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：股转\DVP改革\违约处置（T+2） 对比数据')
        ...
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= '6'  " \
                         "and  briefid in('005_003_113','005_004_027') and regid in('GZ11721601','0899065379'" \
                         ",'GZ11721600' ) and" \
                         " stkid in('871238','830928','810002','430529') ".format(year, begintime, endtime)
        ...
        # 断言
        if not final_result :
            logger().info('股转\DVP改革\违约处置（T+2） 对比数据无异常')
            assert True
        else:
            logger().error('股转\DVP改革\违约处置（T+2） 对比数据异常')
