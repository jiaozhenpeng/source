...
class ContrastRestrictedShares(unittest.TestCase):
    ...
    股转\DVP改革\备付金改革
    """
    yaml = BaseAction().read_yaml(path=PathConfig().share_reconciliation())['DVP']['BFJGG']
    ...
    def test_restricted_shares(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：股转\DVP改革\备付金改革 对比数据')
        ...
        # 查询sql
        exchangemessage_sql = " select * from exchangemessage where exchid='6' and messagedate={} and " \
                              "messagetype in('01','03')".format(begintime[0:8],)
        ...
        # 断言
        if not final_result :
            logger().info('股转\DVP改革\备付金改革 对比数据无异常')
            assert True
        else:
            logger().error('股转\DVP改革\备付金改革 对比数据异常')
