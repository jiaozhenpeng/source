...
class BlockTrade(unittest.TestCase):
    """
    股转\定向可转债\转股\债转股有零债资金
    """
    yaml = BaseAction().read_yaml(path=PathConfig().share_reconciliation())['dxkzz']['ylzzj']
    ...
    def test_block_trade(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：股转\定向可转债\转股\债转股有零债资金 准备数据')
        ...
        if not sql_result:
            logger().info('股转\定向可转债\转股\债转股有零债资金 准备数据完成')
            assert True
        else:
            logger().error('股转\定向可转债\转股\债转股有零债资金 准备数据异常')
