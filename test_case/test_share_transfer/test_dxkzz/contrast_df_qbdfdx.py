...
class ContrastEtfSplit(unittest.TestCase):
    ...
    股转\定向可转债\兑付\全部兑付兑息
    """
    yaml = BaseAction().read_yaml(path=PathConfig().share_reconciliation())['dxkzz']['qbdfdx']
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：股转\定向可转债\兑付\全部兑付兑息 对比数据')
        ...
        if not final_result and not stklist_database:
            logger().info('股转\定向可转债\兑付\全部兑付兑息 对比数据无异常')
            assert True
        else:
            logger().error('股转\定向可转债\兑付\全部兑付兑息 对比数据异常')
