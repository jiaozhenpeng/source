...
class ContrastEtfSplit(unittest.TestCase):
    ...
    股转\定向可转债\兑付\冻结兑付兑息
    """
    yaml = BaseAction().read_yaml(path=PathConfig().share_reconciliation())['dxkzz']['djdfdx']
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：股转\定向可转债\兑付\冻结兑付兑息 对比数据')
        ...
        # 查询sql
        stklist_sql = "select * from STKLIST where exchid='9' and stkid in('810017') and " \
                      "offerregid in('GZ11721600')"
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= '9'  " \
                         "and stkid in ('810017') ".format(year,begintime,endtime)
        stklisthis_sql = "select * from STKLIST{} where occurtime={}  and exchid= '9'  " \
                         "and stkid in ('810017') and offerregid in('GZ11721600')".format(year,begintime)
        ...
        if not final_result :
            logger().info('股转\定向可转债\兑付\冻结兑付兑息 对比数据无异常')
            assert True
        else:
            logger().error('股转\定向可转债\兑付\冻结兑付兑息 对比数据异常')
