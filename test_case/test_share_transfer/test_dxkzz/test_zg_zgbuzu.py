...
class BlockTrade(unittest.TestCase):
    ...
    def test_block_trade(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：股转\定向可转债\转股\债转股所有债券不足一股 准备数据')
        ...
        if not sql_result:
            logger().info('股转\定向可转债\转股\债转股所有债券不足一股 准备数据完成')
            assert True
        else:
            logger().error('股转\定向可转债\转股\债转股所有债券不足一股 准备数据异常')
