...
class ContrastEtfSplit(unittest.TestCase):
    ...
    股转\定向可转债\兑息\全部兑息
    """
    yaml = BaseAction().read_yaml(path=PathConfig().share_reconciliation())['dxkzz']['qbdx']
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：股转\定向可转债\兑息\全部兑息 对比数据')
        ...
        # 查询sql
        stklist_sql = "select * from STKLIST where exchid='9' and stkid in('810021') and " \
        ...
        if not final_result :
            ...
            logger().error('股转\定向可转债\兑息\全部兑息 对比数据异常')
