...
class ContrastGeneralLiquidation(unittest.TestCase):
    ...
    沪权\对冲
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_quan())['Hedging']
    ...
    def test_general_liquid(self):
        ...
        futuretradinglog_sql = "select * from futuretradinglog{}  where reckoningtime>={} and reckoningtime<={} and " \
                               "exchid='X' and regid='A117212005' and stkid in('10002508')".format(year, begintime, endtime)
        futurepositionhis_sql = "select * from futureposition{} WHERE regid='A117212005'  and occurtime={} and stkid " \
                             "in(10002508)".format(year, begintime)
        futurepositiondetailhis_sql = "select * from futurepositiondetail{} WHERE regid='A117212005'  and occurtime={} " \
                                   "and stkid in(10002508)".format(year, begintime)
        futureposition_sql = "select * from futureposition WHERE regid='A117212005'   and stkid  in(10002508) "
        ...
        if not final_result:
            logger().info('沪权\对冲 对比数据无异常')
            assert True
        else:
            logger().error('沪权\对冲 对比数据异常')
