...
class ReserveConversion(unittest.TestCase):
    """
    沪权\备兑转换
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_quan())['ReserveConversion']

    def test_reserve_conversion(self):
        """
        沪权\备兑转换
        :return:
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪权\备兑转换 准备数据')
        ...
        if not sql_result:
            logger().info('沪权\备兑转换 准备数据完成')
            assert True
        else:
            logger().error('沪权\备兑转换 准备数据异常')
