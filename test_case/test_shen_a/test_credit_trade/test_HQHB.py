...
class CCB(unittest.TestCase):
    ...
    深A\融资融券\还券划拨
    """
    yaml = BaseAction().read_yaml(path=PathConfig().shen_a())['CreditTrade']['CreditHQHB']
    ...
    def test_CCB(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：深A\融资融券\还券划拨 准备数据')
        ...
        if not sql_result:
            logger().info('深A\融资融券\还券划拨 准备数据完成')
            assert True
        else:
            logger().error('深A\融资融券\还券划拨 准备数据异常')
