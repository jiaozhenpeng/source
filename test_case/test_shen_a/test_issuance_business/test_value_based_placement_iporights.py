...
class ValueBasedPlacement(unittest.TestCase):
    """
    深A\发行业务\按值配售\权益发放
    """
    ...
    def test_value_based_placement(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：深A\发行业务\按值配售\权益发放 准备数据')
        ...
        if not sql_result:
            logger().info('深A\发行业务\按值配售\权益发放 准备数据完成')
            assert True
        else:
            logger().error('深A\发行业务\按值配售\权益发放 准备数据异常')
