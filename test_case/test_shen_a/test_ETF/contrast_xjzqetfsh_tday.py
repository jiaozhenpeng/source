...
class ContrastBondDistribution(unittest.TestCase):
    ...
    """
    yaml = BaseAction().read_yaml(path=PathConfig().shen_a())['XJZQETFSH']['Tday']
    ...
    def test_bond_distribution(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：深A\ETF申赎\全现金申赎\现金债券ETF\Tday 对比数据')
        ...
        stklist_sql = "select * from STKLIST where  EXCHID = '1' and REGID in ('0999900007'," \
                      "'0666600007') and stkid in('159650','159651') and DESKID = '077011'"
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= '1' and" \
                         "  stkid in('159650','159651','159900')  and contractnum in('M400000002'," \
                         "'GN000006','MA00000002','GA000006')".format(year, begintime, endtime)
        unprocessedreckoningresult_sql = "select * from unprocessedreckoningresult where knocktime={}  and exchid='1' " \
                                         "and contractnum in('M400000002','GN000006','MA00000002','GA000006')".format(begintime,)
        unprocessedreckoningresulthis_sql = "select * from unprocessedreckoningresulthis where knocktime={}  and " \
                                            "exchid='1' and contractnum in('M400000002','GN000006','MA00000002','GA000006')".format(begintime,)
        etfcashrefillwait_sql = "select * from etfcashrefillwait where exchid='1' and fundcode in('159650','159651') " \
                                "and occurtime = {}".format(begintime,)
        # 数据库数据
        stklist_database = base.stklist_sort(oracle.dict_data(stklist_sql))
        tradinglog_database = base.tradinglog_sort(oracle.dict_data(tradinglog_sql))
        unprocessedreckoningresult_database = base.unprocessedreckoningresult_sort1(oracle.dict_data(unprocessedreckoningresult_sql))
        unprocessedreckoningresulthis_database = base.unprocessedreckoningresulthis_sort1(oracle.dict_data(unprocessedreckoningresulthis_sql))
        ...
        unprocessedreckoningresult_excel = base.unprocessedreckoningresult_sort1(excel.read_excel('unprocessedreckoningresult'))
        unprocessedreckoningresulthis_excel = base.unprocessedreckoningresulthis_sort1(excel.read_excel('unprocessedreckoningresulthis'))
        ...
        if not final_result:
            logger().info('深A\ETF申赎\全现金申赎\现金债券ETF\Tday 对比数据无异常')
            assert True
        else:
            logger().error('深A\ETF申赎\全现金申赎\现金债券ETF\Tday 对比数据异常')
