...
class BondDistribution(unittest.TestCase):
    ...
    """
    yaml = BaseAction().read_yaml(path=PathConfig().shen_a())['DSCETFSH']['Tday']
    ...
    def test_bond_distribution(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：深A\ETF申赎\单市场ETF申赎\T日 准备数据')
        ...
        if not sql_result:
            logger().info('深A\ETF申赎\单市场ETF申赎\T日 准备数据完成')
            assert True
        else:
            logger().error('深A\ETF申赎\单市场ETF申赎\T日 准备数据异常')
