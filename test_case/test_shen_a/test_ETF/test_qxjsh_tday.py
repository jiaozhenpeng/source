...
class BondDistribution(unittest.TestCase):
    ...
    深A\ETF申赎\全现金申赎\Tday
    """
    yaml = BaseAction().read_yaml(path=PathConfig().shen_a())['XJETFSH']['Tday']
    ...
    def test_bond_distribution(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：深A\ETF申赎\全现金申赎\Tday 准备数据')
        ...
        if not sql_result:
            logger().info('深A\ETF申赎\全现金申赎\Tday 准备数据完成')
            assert True
        else:
            logger().error('深A\ETF申赎\全现金申赎\Tday 准备数据异常')
