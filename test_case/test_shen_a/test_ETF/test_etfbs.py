...
class BondDistribution(unittest.TestCase):
    ...
    """
    yaml = BaseAction().read_yaml(path=PathConfig().shen_a())['etfbs']
    ...
    def test_bond_distribution(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：F:\source\用例数据\深A\ETF申赎\买卖 准备数据')
        ...
        if not sql_result:
            logger().info('F:\source\用例数据\深A\ETF申赎\买卖 准备数据完成')
            assert True
        else:
            logger().error('F:\source\用例数据\深A\ETF申赎\买卖 准备数据异常')
