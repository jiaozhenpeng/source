...
class ContrastRestrictedShares(unittest.TestCase):
    ...
    深A\LOF基金\申购\T日
    """
    yaml = BaseAction().read_yaml(path=PathConfig().shen_a())['lofshengou']['Tday']
    ...
    def test_restricted_shares(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：深A\LOF基金\申购\T日 对比数据')
        ...
        # 查询sql
        unprocessedreckoningresulthis_sql = "select * from unprocessedreckoningresulthis where EXCHID='1' and STKID " \
                                            "in ('160417','160419','160423','160420') and REGID in " \
                                            "('0117322000','0117322001')  and knocktime>={}".format(begintime, )
        unprocessedreckoningresult_sql = "select * from unprocessedreckoningresult where EXCHID='1' and STKID " \
                                         "in ('160417','160419','160423','160420') and REGID in " \
                                         "('0117322000','0117322001') and knocktime>={}".format(begintime, )
        stklist_sql = "select * from STKLIST where EXCHID = '1' and REGID in( '0117322000'," \
                      "'0117322001') and STKID in ('160417','160419','160423','160420') and DESKID = '077011'"
        # 获取数据库数据并排序
        ...
        # 断言
        if not final_result:
            logger().info('深A\LOF基金\申购\T日 对比数据无异常')
            assert True
        else:
            logger().error('深A\LOF基金\申购\T日 对比数据异常')
