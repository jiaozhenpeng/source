...
class DESY(unittest.TestCase):
    ...
    """
    yaml = BaseAction().read_yaml(PathConfig().shen_a())['RealEraPayments']['DESY']
    ...
    def test_DESY(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：实时代收付\货币基金基金收益 准备数据')
        ...
        if not sql_result:
            logger().info('深圳实时代收付\货币基金基金收益 准备数据完成')
            assert True
        else:
            logger().error('深圳实时代收付\货币基金基金收益 准备数据异常')
