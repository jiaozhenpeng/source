...
class BondDistribution(unittest.TestCase):
    ...
    深A\双创可转债
    """
    yaml = BaseAction().read_yaml(path=PathConfig().shen_a())['shuangchuangbond']['Tday']
    ...
    #     """
    #     logger().info('-------------------------------')
    #     logger().info('开始执行：深A\双创可转债 准备数据')
    ...
    #     if not sql_result:
    #         logger().info('深A\双创可转债 准备数据完成')
    #         assert True
    #     else:
    #         logger().error('深A\双创可转债 准备数据异常')
