...
class ContrastRestrictedShares(unittest.TestCase):
    ...
    深A\信用保护合约\CSTZ提前终止
    """
    yaml = BaseAction().read_yaml(path=PathConfig().shen_a())['XYBHHY']['CSTZ']
    ...
    def test_restricted_shares(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：深A\信用保护合约\CSTZ提前终止 对比数据')
        ...
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid='1' and " \
                         "briefid in('005_005_081','005_005_082')".format(year,begintime,endtime)
        # 获取数据库数据并排序
        ...
        if not tradinglog_result :
            logger().info('深A\信用保护合约\CSTZ提前终止 对比数据无异常')
            assert True
        else:
            logger().error('深A\信用保护合约\CSTZ提前终止 对比数据异常')
