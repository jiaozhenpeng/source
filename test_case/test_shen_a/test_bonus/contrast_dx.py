...
class ContrastDateOfPayment(unittest.TestCase):
    """
     深A\红股红利\兑息\兑息当日
    """
    yaml = BaseAction().read_yaml(path=PathConfig().shen_a())['duixi']['dangriduixi']
    ...
    def test_date_of_payment(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行： 深A\红股红利\兑息\兑息当日 对比数据')
        ...
        stklist_sql = "select * from STKLIST{} where EXCHID = '1' and REGID in( '0117212000','0117212001','0117252000'," \
                      "'0117252001') and STKID = '101825' and DESKID = '077011' and OCCURTIME = {}".format(year,begintime)
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= '1' and" \
                         "  stkid = '101825' and briefid in('005_005_002')".format(year,begintime,endtime)
        stkcheckin_sql = "select * from stkcheckin where EXCHID ='1' and STKID ='101825' and OCCURTIME ={} ".format(begintime)
        ...
        if not final_result:
            logger().info(' 深A\红股红利\兑息\兑息当日 对比数据无异常')
            assert True
        else:
            logger().error(' 深A\红股红利\兑息\兑息当日 对比数据异常')
