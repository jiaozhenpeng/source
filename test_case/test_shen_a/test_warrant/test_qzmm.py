...
class StockPurchase(unittest.TestCase):
    """
    F:\source\用例数据\深A\权证行权\买卖
    """
    yaml = BaseAction().read_yaml(path=PathConfig().shen_a())['QZMM']
    ...
    def test_stock_purchase(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：F:\source\用例数据\深A\权证行权\买卖 准备数据')
        ...
        if not sql_result:
            logger().info('F:\source\用例数据\深A\权证行权\买卖 准备数据完成')
            assert True
        else:
            logger().error('F:\source\用例数据\深A\权证行权\买卖 准备数据异常')
