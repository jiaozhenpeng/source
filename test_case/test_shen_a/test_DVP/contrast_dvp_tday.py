...
class ContrastRestrictedShares(unittest.TestCase):
    ...
    深A\DVP改革\可售交收冻结（T日）
    """
    yaml = BaseAction().read_yaml(path=PathConfig().shen_a())['DVP']['Tday']
    ...
    def test_restricted_shares(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：深A\DVP改革\可售交收冻结（T日） 对比数据')
        ...
        # 断言
        if not final_result :
            logger().info('深A\DVP改革\可售交收冻结（T日） 对比数据无异常')
            assert True
        else:
            logger().error('深A\DVP改革\可售交收冻结（T日） 对比数据异常')
