...
class ContrastStockTrading(unittest.TestCase):
    ...
    深A\股票买入
    """
    yaml = BaseAction().read_yaml(path=PathConfig().shen_a())['StockTrade']
    ...
    def test_Stock_Trading(self):
        ...
        # 查询sql
        account_sql = "select * from  account where acctid ='000011728200' and currencyid='00'"
        stklist_sql = "select * from stklist where exchid='1' and stkid='000001' and regid='0117282000' "
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= '1' and " \
                         "  regid='0117282000' and stkid in ('000001') and briefid in('005_001_001')".format(year,begintime,endtime)
        # 数据库数据
        ...
        if not final_result:
            logger().info('深A\股票买入 对比数据无异常')
            assert True
        else:
            logger().error('深A\股票买入 对比数据异常')
