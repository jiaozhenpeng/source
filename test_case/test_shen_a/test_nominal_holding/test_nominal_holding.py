...
class NominalHolding(unittest.TestCase):
    """
    名义持有明细股份管理
    """
    yaml = BaseAction().read_yaml(PathConfig().shen_a())['NominalHolding']
    ...
    def test_NominalHolding(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：名义持有明细股份管理 准备数据')
        ...
        if not sql_result:
            logger().info('名义持有明细股份管理 准备数据完成')
            assert True
        else:
            logger().error('名义持有明细股份管理 准备数据异常')
