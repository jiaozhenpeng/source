...
class VirtualShareholders(unittest.TestCase):
    """
    深A\GDR数据\虚拟股东
    """
    yaml = BaseAction().read_yaml(path=PathConfig().shen_a())['GDR']['VirtualShareholders']

    def test_virtual_shareholders(self):
        """
        深A\GDR数据\虚拟股东
        :return:
        """
        logger().info('-------------------------------')
        logger().info('开始执行：深A\GDR数据\虚拟股东 准备数据')
        ...
        if not sql_result:
            logger().info('深A\GDR数据\虚拟股东 准备数据完成')
            assert True
        else:
            logger().error('深A\GDR数据\虚拟股东 准备数据异常')
