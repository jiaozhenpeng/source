...
class StockPurchase(unittest.TestCase):
    """
    深A\债券质押式回购\质押出库\固收平台质押出库
    """
    yaml = BaseAction().read_yaml(path=PathConfig().shen_a())['bondrepo']['ZYCK13']
    ...
    def test_stock_purchase(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：深A\债券质押式回购\质押出库\固收平台质押出库 准备数据')
        ...
        if not sql_result:
            logger().info('深A\债券质押式回购\质押出库\固收平台质押出库 准备数据完成')
            assert True
        else:
            logger().error('深A\债券质押式回购\质押出库\固收平台质押出库 准备数据异常')
