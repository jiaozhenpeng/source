...
class ContrastEtfSplit(unittest.TestCase):
    ...
    沪银行\上市
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_bank())['shangshi']
    ...
    def test_etf_split(self):
        ...
        # 查询sql
        stklist_sql = "select * from STKLIST where exchid='C' and stkid in('012280566') "
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= 'C'  " \
                         "and stkid in ('012280566') ".format(year, begintime, endtime)
        ...
        if not final_result:
            logger().info('沪银行\上市 对比数据无异常')
            assert True
        else:
            logger().error('沪银行\上市 对比数据异常')
