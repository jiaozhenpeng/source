...
class ContrastEtfSplit(unittest.TestCase):
    ...
    沪银行\买卖交易\买方银行间交易作废
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_bank())['ordinary_trade']['buy_bankfailed']
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪银行\买卖交易\买方银行间交易作废 对比数据')
        ...
        # 查询sql
        stklist_sql = "select * from STKLIST where exchid='C' and stkid in('042200012') "
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= 'C'  " \
                         "and (stkid in ('042200012') or reckoningamt=0 ) and ordertype='B' ".format(year, begintime, endtime)
        ...
        if not (tradinglog_database and stklist_database):
            ...
            logger().error('沪银行\买卖交易\买方银行间交易作废 对比数据异常')
