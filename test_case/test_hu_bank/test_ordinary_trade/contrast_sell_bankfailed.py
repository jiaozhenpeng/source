...
class ContrastEtfSplit(unittest.TestCase):
    ...
    沪银行\买卖交易\卖方银行间交易作废
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_bank())['ordinary_trade']['sell_bankfailed']
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪银行\买卖交易\卖方银行间交易作废 对比数据')
        ...
        # 查询sql
        stklist_sql = "select * from STKLIST where exchid='C' and stkid in('092103012') "
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= 'C'  " \
                         "and (stkid in ('092103012') or (reckoningamt=0 and briefid='005_005_093' ) ) and ordertype='S' ".format(year, begintime, endtime)
        stklisthis_sql = "select * from STKLIST{} where exchid='C' and stkid in('092103012') " \
        ...
        # 断言
        final_result =    stklist_result + stklisthis_result
        if not (final_result and tradinglog_database):
            logger().info('沪银行\买卖交易\卖方银行间交易作废 对比数据无异常')
            assert True
        else:
            logger().error('沪银行\买卖交易\卖方银行间交易作废 对比数据异常')
