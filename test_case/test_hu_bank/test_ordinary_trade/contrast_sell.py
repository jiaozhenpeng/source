...
class ContrastEtfSplit(unittest.TestCase):
    ...
    沪银行\买卖交易\卖方交收成功
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_bank())['ordinary_trade']['sell']
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪银行\买卖交易\卖方交收成功 对比数据')
        ...
        # 查询sql
        stklist_sql = "select * from STKLIST where exchid='C' and stkid in('092000013') "
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= 'C'  " \
                         "and stkid in ('092000013') ".format(year, begintime, endtime)
        stklisthis_sql = "select * from STKLIST{} where exchid='C' and stkid in('092000013') " \
        ...
        if not (final_result and stklist_database):
            logger().info('沪银行\买卖交易\卖方交收成功 对比数据无异常')
            assert True
        else:
            logger().error('沪银行\买卖交易\卖方交收成功 对比数据异常')
