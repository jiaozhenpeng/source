...
class ContrastEtfSplit(unittest.TestCase):
    ...
    沪银行\买卖交易\买方交收成功
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_bank())['ordinary_trade']['buy']
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪银行\买卖交易\买方交收成功 对比数据')
        ...
        if not final_result:
            logger().info('沪银行\买卖交易\买方交收成功 对比数据无异常')
            assert True
        else:
            logger().error('沪银行\买卖交易\买方交收成功 对比数据异常')
