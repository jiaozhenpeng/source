...
class ContrastEtfSplit(unittest.TestCase):
    ...
    沪银行\兑付\主股东
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_bank())['qpdf']['zhugudong']
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪银行\兑付\主股东 对比数据')
        ...
        # 查询sql
        stklist_sql = "select * from stklist where exchid='C' and  stkid='012103064' "
        stklisthis_sql = " select * from    stklist{} where occurtime={} and   exchid= 'C'  and " \
                         "stkid in ('012103064')".format(year,begintime)
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= 'C'  " \
                         "and stkid in ('012103064') ".format(year, begintime, endtime)
        ...
        # 断言
        final_result =   tradinglog_result + stklisthis_result
        if not (final_result and stklist_database):
            logger().info('沪银行\兑付\主股东 对比数据无异常')
            assert True
        else:
            logger().error('沪银行\兑付\主股东 对比数据异常')
