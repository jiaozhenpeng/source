...
class ContrastEtfSplit(unittest.TestCase):
    ...
    沪银行\兑息\主股东
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_bank())['qpdx']['zhugudong']
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪银行\兑息\主股东 对比数据')
        ...
        # 查询sql
        stklist_sql = "select * from stklist where exchid='C' and  stkid='092100010' "
        stklisthis_sql = " select * from    stklist{} where occurtime={} and   exchid= 'C'  and " \
                         "stkid in ('092100010')".format(year, begintime)
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= 'C'  " \
                         "and stkid in ('092100010') ".format(year, begintime, endtime)
        ...
        if not final_result:
            logger().info('沪银行\兑息\主股东 对比数据无异常')
            assert True
        else:
            logger().error('沪银行\兑息\主股东 对比数据异常')
