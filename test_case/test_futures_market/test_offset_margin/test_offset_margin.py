...
class OffsetMargin(unittest.TestCase):
    ...
    期货市场\冲抵保证金
    """
    yaml = BaseAction().read_yaml(path=PathConfig().futures_market())['OffsetMargin']
    ...
    def test_offset_margin(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：期货市场\冲抵保证金 准备数据')
        txt_path = self.yaml['txtPath']
        txt = TxtOperation(txt_path)
        # 创建文件
        txt_result = txt.creat_txt('otherfund')
        if txt_result is False:
            logger().error('otherfund.txt文件创建失败')
            assert False, 'otherfund.txt文件创建失败'
        sql_path = self.yaml['sqlPath']
        sql = BaseAction().read_sql(sql_path)
        oracle = OracleDatabase()
        sql_result = oracle.update_sql(*sql)
        if not sql_result:
            logger().info('期货市场\冲抵保证金 准备数据完成')
            assert True
        else:
            logger().error('期货市场\冲抵保证金 准备数据异常')
