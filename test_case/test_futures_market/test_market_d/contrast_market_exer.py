...
class ContrastFuturesOpen(unittest.TestCase):
    ...
    def test_futures_opening(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：商品期权行权后自动对冲\D市场 对比数据')
        test_yaml = ContrastFuturesOpen().yaml['market_d']['MarketExer']
        ...
        if not futureposition_result and not futurepositiondetail_result and not futuretradinglog_result:
            logger().info('商品期权行权后自动对冲\D市场 对比数据无异常')
            assert True
        else:
            logger().error('商品期权行权后自动对冲\D市场 对比数据异常')
