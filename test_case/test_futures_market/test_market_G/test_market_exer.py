...
class FuturesOpen(unittest.TestCase):
    ...
    def test_futures_opening(self):
        ...
        '''
        logger().info('-------------------------------')
        logger().info('开始执行：期货市场\商品期权行权后自动对冲\G市场 准备数据')
        ...
        if not oracle_result:
            logger().info('期货市场\商品期权行权后自动对冲\G市场 准备数据完成')
            assert True
        else:
            logger().error('期货市场\商品期权行权后自动对冲\G市场 准备数据失败：{}'.format(oracle_result))
