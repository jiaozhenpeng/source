...
class Contrast1Business(unittest.TestCase):
    """
    深港\\买卖T+1
    """
    ...
    def test_shen_business(self):
        logger().info('-------------------------------')
        logger().info('开始执行：深港\\买卖T+1日清算 对比数据')
        excel_path = self.yaml['excelPath1']
        ...
        if not stklist_result and not unprocessedreckoningresult_result:
            logger().info('深港\\买卖T+1日清算 对比数据无异常')
            assert True
        else:
            logger().error('深港\\买卖T+1日清算 对比数据异常')
