...
class ContrastEtfSplit(unittest.TestCase):
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：深银行\普通交易\买入 对比数据')
        ...
        # 查询sql
        stklist_sql = "select * from STKLIST where exchid='B' and stkid in('092118002','092118003') "
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= 'B'  " \
                         "and stkid in ('092118002','092118003') ".format(year,begintime,endtime)
        ...
        if not final_result:
            logger().info('深银行\普通交易\买入 对比数据无异常')
            assert True
        else:
            logger().error('深银行\普通交易\买入 对比数据异常')
