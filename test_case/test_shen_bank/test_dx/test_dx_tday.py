...
class SZYH(unittest.TestCase):
    ...
    def test_b_turn_h(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：F:\source\用例数据\深银行\兑息\普通兑息\T日 准备数据')
        ...
        if not sql_result:
            logger().info('F:\source\用例数据\深银行\兑息\普通兑息\T日 准备数据完成')
            assert True
        else:
            logger().error('F:\source\用例数据\深银行\兑息\普通兑息\T日 准备数据异常')
