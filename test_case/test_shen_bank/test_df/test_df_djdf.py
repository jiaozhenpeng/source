...
class SZYH(unittest.TestCase):
    ...
    深银行\兑付\冻结兑付
    """
    yaml =  BaseAction().read_yaml(path=PathConfig().shen_bank())['df']['djdf']
    def test_b_turn_h(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：深银行\兑付\冻结兑付 准备数据')
        ...
        if not sql_result:
            logger().info('深银行\兑付\冻结兑付 准备数据完成')
            assert True
        else:
            logger().error('深银行\兑付\冻结兑付 准备数据异常')
