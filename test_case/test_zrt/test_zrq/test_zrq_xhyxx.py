...
class ZlpsCJ(unittest.TestCase):
    ...
    转融通\转融券\流通股转融券\新合约
    """
    yaml = BaseAction().read_yaml(path=PathConfig().zrt())['ZRQ_PT']['tday']
    ...
    def test_general_liquidation(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：转融通\转融券\流通股转融券\新合约 准备数据')
        ...
        if not sql_result:
            logger().info('转融通\转融券\流通股转融券\新合约 准备数据完成')
            assert True
        else:
            logger().error('转融通\转融券\流通股转融券\新合约 准备数据异常')
