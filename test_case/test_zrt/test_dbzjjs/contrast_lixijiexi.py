...
class ContrastVirtualExer(unittest.TestCase):
    ...
    转融通\担保资金结算\利息结算
    """
    yaml = BaseAction().read_yaml(path=PathConfig().zrt())['DBZJJS']['LXJS']
    ...
    def test_virtualexer(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：转融通\担保资金结算\利息结算 对比数据')
        ...
        tradinglog_sql = "select b.INTERIORDESC,a.* from tradinglog{} a ,briefdefine b where a.briefid=b.briefid  and" \
                         " a.briefid='002_005_009' and acctid='555000000003' and a.reckoningtime>={} and " \
        ...
        if not final_result:
            logger().info('转融通\担保资金结算\利息结算 对比数据无异常')
            assert True
        else:
            logger().error('转融通\担保资金结算\利息结算 对比数据异常')
