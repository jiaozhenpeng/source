...
class ZlpsCJ(unittest.TestCase):
    ...
    转融通\担保品对账\担保证券对账
    """
    yaml = BaseAction().read_yaml(path=PathConfig().zrt())['DBPDZ']['dbzq']
    ...
    def test_general_liquidation(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：转融通\担保品对账\担保证券对账 准备数据')
        ...
        if not sql_result:
            logger().info('转融通\担保品对账\担保证券对账 准备数据完成')
            assert True
        else:
            logger().error('转融通\担保品对账\担保证券对账 准备数据异常')
