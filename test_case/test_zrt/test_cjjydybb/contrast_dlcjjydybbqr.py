...
class ContrastVirtualExer(unittest.TestCase):
    ...
    转融通\独立出借人交易单元报备
    """
    yaml = BaseAction().read_yaml(path=PathConfig().zrt())['DLCJRJYDYBBQR']
    ...
    def test_virtualexer(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：转融通\独立出借人交易单元报备 对比数据')
        ...
        # 查询SQL
        rc_LendDeskApply_sql = "select * from rc_LendDeskApply where exptime>={} and exptime<={} and lendertype='0' ".format(begintime,endtime)
        rc_LendDeskCfm_sql = "select * from  rc_LendDeskCfm where lendertype='0' and senddate={} ".format(shortbegintime, )
        ...
        if not final_result:
            logger().info('转融通\独立出借人交易单元报备 对比数据无异常')
            assert True
        else:
            logger().error('转融通\独立出借人交易单元报备 对比数据异常')
