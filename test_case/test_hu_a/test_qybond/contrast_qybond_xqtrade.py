...
class ContrastEtfSplit(unittest.TestCase):
    ...
    沪A\企业债\企业债买卖
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['qybond']['trade']
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\企业债\企业债买卖 对比数据')
        ...
        if not final_result:
            logger().info('沪A\企业债\企业债买卖 对比数据无异常')
            assert True
        else:
            logger().error('沪A\企业债\企业债买卖 对比数据异常')
