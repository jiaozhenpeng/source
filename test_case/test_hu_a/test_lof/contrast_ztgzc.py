...
class ContrastRestrictedShares(unittest.TestCase):
    ...
    def test_restricted_shares(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\上证lof\lof跨市场转托管转出 对比数据')
        ...
        # 查询sql
        tradinglog_sql = "select * from tradinglog{} where EXCHID='0' and STKID in ('501061','501062','501063') and " \
                         "REGID in ('A117322000','A117322001') and reckoningtime>={} " \
                         "and reckoningtime<={}".format(year,begintime,endtime )
        stklist_sql = "select * from   stklist where regid in ('A117322000','A117322001') and stkid  in('501061','501062','501063')"
        stklisthis_sql = "select * from   stklist{}  where occurtime={} and regid in ('A117322000','A117322001') and stkid " \
                         " in('501061','501062','501063')".format(year,begintime)
        # 获取数据库数据并排序
        ...
        # 断言
        if not final_result :
            logger().info('沪A\上证lof\lof跨市场转托管转出 对比数据无异常')
            assert True
        else:
            logger().error('沪A\上证lof\lof跨市场转托管转出 对比数据异常')
