...
class ContrastRestrictedShares(unittest.TestCase):
    ...
    沪A\上证lof\申购\Tday
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['lofshengou']['Tday']
    ...
    def test_restricted_shares(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\上证lof\申购\Tday 对比数据')
        ...
        stklist_sql = "select * from STKLIST where EXCHID = '0' and REGID in( 'A117322000'," \
                      "'A117322001') and STKID in ('501057','501058') and DESKID = '00W40'"
        # 获取数据库数据并排序
        ...
        if not final_result and not stklist_database:
            logger().info('沪A\上证lof\申购\Tday 对比数据无异常')
            assert True
        else:
            logger().error('沪A\上证lof\申购\Tday 对比数据异常')
