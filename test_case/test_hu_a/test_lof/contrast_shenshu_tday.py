...
class ContrastRestrictedShares(unittest.TestCase):
    ...
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['lofshenshu']['Tday']
    ...
    def test_restricted_shares(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\上证lof\当天同时申购赎回\Tday 对比数据')
        ...
        stklist_sql = "select * from STKLIST where EXCHID = '0' and REGID in( 'A117322000'," \
                      "'A117322001') and STKID in ('501027') and DESKID = '00W40'"
        # 获取数据库数据并排序
        ...
        # 断言
        if not final_result  :
            logger().info('沪A\上证lof\当天同时申购赎回\Tday 对比数据无异常')
            assert True
        else:
            logger().error('沪A\上证lof\当天同时申购赎回\Tday 对比数据异常')
