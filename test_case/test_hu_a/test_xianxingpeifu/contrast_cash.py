...
class ContrastEtfSplit(unittest.TestCase):
    ...
    沪A\先行赔付\权益登记日
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['xianxingpeifu']['cash']
    ...
    def test_etf_split(self):
        ...
        # 查询sql
        regrights_sql = "select * from regrights where exchid='0' and stkid in('688087') "
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= '0'  " \
                         "and stkid in ('688086','688087')  and briefid in('005_004_114','005_005_097' )".format(year, begintime, endtime)
        ...
        if not final_result and not regrights_database:
