...
class Futures(unittest.TestCase):
    ...
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['DX']['DX']
    ...
    def test_futures(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：F:\source\用例数据\沪A\兑息\兑息 准备数据')
        ...
        if not sql_result:
            logger().info('F:\source\用例数据\沪A\兑息\兑息 准备数据完成')
            assert True
        else:
            logger().error('F:\source\用例数据\沪A\兑息\兑息 准备数据异常')
