...
class ContrastEtfSplit(unittest.TestCase):
    """
    沪A\公司债\固定收益RTGS 债券交易
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['companybond']['gsrtgstrade']
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\公司债\固定收益RTGS 债券交易 对比数据')
        ...
        # 查询sql
        stklist_sql = "select * from STKLIST where exchid='0' and stkid in('177353','177354') and " \
        ...
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= '0'  " \
                         "and stkid in('177353','177354') and briefid in('005_001_001','005_002_001')".format(year,begintime,endtime)
        ...
        if not final_result:
            logger().info('沪A\公司债\固定收益RTGS 债券交易 对比数据无异常')
            assert True
        else:
            logger().error('沪A\公司债\固定收益RTGS 债券交易 对比数据异常')
