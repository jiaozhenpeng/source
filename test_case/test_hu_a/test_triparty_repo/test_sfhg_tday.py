...
# 沪A\上海三方回购\T日（680，684，685）
class TripartyReposfcs(unittest.TestCase):
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['tripartyrepo']['Tday']
    ...
    def test_block_trade(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\上海三方回购\T日 准备数据')
        ...
        if not sql_result:
            logger().info('沪A\上海三方回购\T日 准备数据完成')
            assert True
        else:
            logger().error('沪A\上海三方回购\T日 准备数据异常')
