...
class EquityDistribution(unittest.TestCase):
    """
    沪A\上海配股配债\T日权益发放
    """
    ...
    def test_equity_distribution(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\上海配股配债\T日权益发放 准备数据')
        ...
        if not sql_result:
            logger().info('沪A\上海配股配债\T日权益发放 准备数据完成')
            assert True
        else:
            logger().error('沪A\上海配股配债\T日权益发放 准备数据异常')
