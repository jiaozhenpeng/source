...
class AfterHoursDealing(unittest.TestCase):
    """
    沪A\科创板\盘后交易
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['ScientificInnovationBoard']['AfterHoursDealing']

    def test_after_hours_dealing(self):
        """
        沪A\科创板\盘后交易
        :return:
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\科创板\盘后交易 准备数据')
        ...
        if not sql_result:
            logger().info('沪A\科创板\盘后交易 准备数据完成')
            assert True
        else:
            logger().error('沪A\科创板\盘后交易 准备数据异常')
