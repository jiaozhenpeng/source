...
class ContrastStockPurchase(unittest.TestCase):
    ...
    沪A\科创板\普通交易
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['ScientificInnovationBoard']['OrdinaryTransaction']
    ...
    def test_stock_purchase(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\科创板\普通交易 对比数据')
        ...
        # 查询sql
        stklist_sql = "select * from STKLIST where EXCHID = '0' and  offerregid='A117212000' and stkid in('688001','688004')"
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= '0'" \
                         "  and stkid in('688001','688004')".format(year,begintime,endtime)
        # 数据库数据
        ...
        if not final_result:
            logger().info('沪A\科创板\普通交易 对比数据无异常')
            assert True
        else:
            logger().error('沪A\科创板\普通交易 对比数据异常')
