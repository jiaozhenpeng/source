...
class Futures(unittest.TestCase):
    ...
    F:\source\用例数据\沪A\兑付
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['DF']
    ...
    def test_futures(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：F:\source\用例数据\沪A\兑付 准备数据')
        ...
        if not sql_result:
            logger().info('F:\source\用例数据\沪A\兑付 准备数据完成')
            assert True
        else:
            logger().error('F:\source\用例数据\沪A\兑付 准备数据异常')
