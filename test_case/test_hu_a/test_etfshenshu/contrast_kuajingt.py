...
class ContrastEtfSplit(unittest.TestCase):
    """
    沪A\ETF申赎\跨境ETFT+0非担保交收\T日
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['ETFSH']['kuajingT']
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\ETF申赎\跨境ETFT+0非担保交收\T日 对比数据')
        ...
        if not final_result:
            logger().info('沪A\ETF申赎\跨境ETFT+0非担保交收\T日 对比数据无异常')
            assert True
        else:
            logger().error('沪A\ETF申赎\跨境ETFT+0非担保交收\T日 对比数据异常')
