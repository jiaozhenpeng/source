...
class StockPurchase(unittest.TestCase):
    ...
    沪A\ETF申赎\债券ETF实物申赎
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['ETFSH']['zhaiquanshiwuT']
    ...
    def test_stock_purchase(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\ETF申赎\债券ETF实物申赎 准备数据')
        ...
        if not sql_result:
            logger().info('沪A\ETF申赎\债券ETF实物申赎 准备数据完成')
            assert True
        else:
            logger().error('沪A\ETF申赎\债券ETF实物申赎 准备数据异常')
