...
class ContrastEtfSplit(unittest.TestCase):
    ...
    沪A\可转债转股
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['ConvertibleBond']['ShareConversion']
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\可转债转股 对比数据')
        ...
        # 查询sql
        stklist_sql = "select * from STKLIST where exchid='0' and stkid in('110032','600031') "
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= '0'  " \
                         "and stkid in ('110032','600031') ".format(year, begintime, endtime)
        ...
        if not final_result:
            logger().info('沪A\可转债转股 对比数据无异常')
            assert True
        else:
            logger().error('沪A\可转债转股 对比数据异常')
