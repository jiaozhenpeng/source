...
class ContrastRestrictedShares(unittest.TestCase):
    ...
    沪A\各种证券变动\00C
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['ChangesInSecurities']['C']
    ...
    def test_restricted_shares(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\各种证券变动\00C 对比数据')
        ...
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= '0'  " \
                         "and stkid in ('603107','603108','603105','603106') ".format(year, begintime, endtime)
        # 获取数据库数据并排序
        ...
        # 断言
        if not final_result :
            logger().info('沪A\各种证券变动\00C 对比数据无异常')
            assert True
        else:
            logger().error('沪A\各种证券变动\00C 对比数据异常')
