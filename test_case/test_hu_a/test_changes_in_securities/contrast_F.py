...
class ContrastRestrictedShares(unittest.TestCase):
    ...
    沪A\各种证券变动\00F
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['ChangesInSecurities']['F']
    ...
    def test_restricted_shares(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\各种证券变动\00F 对比数据')
        ...
        stklistextend_sql = " select * from   stklistextend where exchid='0' and offerregid in('A117212000'," \
                            "'A117252000' ) and stkid in('603020','603021','603022','603023','603024')"
        stklist_sql = "select * from  stklist where exchid='0' and offerregid in('A117212000','A117252000' ) " \
                      "and stkid in('603020','603021','603022','603023','603024')"
        stklisthis_sql = " select * from stklist{} where occurtime={} and stkid in" \
                         " ('603020','603021','603022','603023','603024') ".format(year,begintime)
        ...
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= '0'  " \
                         "and stkid in ('603020','603021','603022','603023','603024') ".format(year, begintime, endtime)
        # 获取数据库数据并排序
        ...
        # 断言
        if not final_result :
            logger().info('沪A\各种证券变动\00F 对比数据无异常')
            assert True
        else:
            logger().error('沪A\各种证券变动\00F 对比数据异常')
