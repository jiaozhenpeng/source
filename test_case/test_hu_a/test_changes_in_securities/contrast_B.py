...
class ContrastRestrictedShares(unittest.TestCase):
    ...
    def test_restricted_shares(self):
        ...
        stklistextend_sql = " select * from   stklistextend where exchid='0' and offerregid in('A117212000'," \
                            "'A117252000' ) and stkid in('601028','601066','601068','601369','601375')"
        stklist_sql = "select * from  stklist where exchid='0' and offerregid in('A117212000','A117252000' ) " \
                      "and stkid in('601028','601066','601068','601369','601375')"
        stklisthis_sql = " select * from stklist{} where occurtime={} and stkid in ('601028','601066','601068'," \
                         "'601369','601375') ".format(year,begintime)
        stklistextendhis_sql = " select * from stklistextend{} where occurtime={} and stkid in ('601028','601066','601068'," \
                         "'601369','601375') ".format(year,begintime)
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= '0'  " \
                         "and stkid in ('601028','601066','601068','601369','601375') ".format(year, begintime, endtime)
        # 获取数据库数据并排序
