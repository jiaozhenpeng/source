...
class ContrastRestrictedShares(unittest.TestCase):
    ...
    沪A\ETF业务\货币基金权益结转
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['bd100']
    ...
    def test_restricted_shares(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\ETF业务\货币基金权益结转 对比数据')
        ...
        #                     "'A117252000' ) and stkid in('511920')"
        ...
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= '0'  " \
                         "and stkid in ('511920') ".format(year, begintime, endtime)
        # 获取数据库数据并排序
        ...
        final_result = stklist_result + stklisthis_result + tradinglog_result

        # 断言
        if not final_result :
            logger().info('沪A\ETF业务\货币基金权益结转 对比数据无异常')
            assert True
        else:
            logger().error('沪A\ETF业务\货币基金权益结转 对比数据异常')
