...
class ContrastRestrictedShares(unittest.TestCase):
    ...
    货币ETF基金收益，etftbk文件
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['etfjjsy']['etftbk']
    ...
    def test_restricted_shares(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：货币ETF基金收益，etftbk文件 对比数据')
        ...
        # 断言
        if not final_result:
            logger().info('货币ETF基金收益，etftbk文件 对比数据无异常')
            assert True
        else:
            logger().error('货币ETF基金收益，etftbk文件 对比数据异常')
