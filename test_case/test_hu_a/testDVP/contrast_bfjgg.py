...
class ContrastRestrictedShares(unittest.TestCase):
    ...
    沪A\DVP改革\备付金改革
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['DVP']['BFJGG']
    ...
    def test_restricted_shares(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\DVP改革\备付金改革 对比数据')
        ...
        # 查询sql
        exchangemessage_sql = " select * from exchangemessage where exchid='0' and messagedate={} and " \
                              "messagetype in('020','030','050')".format(begintime[0:8],)
        ...
        # 断言
        if not final_result :
            logger().info('沪A\DVP改革\备付金改革 对比数据无异常')
            assert True
        else:
            logger().error('沪A\DVP改革\备付金改革 对比数据异常')
