...
class BlockTrade(unittest.TestCase):
    ...
    沪A\DVP改革\可售交收冻结（T日）
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['DVP']['Tday']
    ...
    def test_block_trade(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\DVP改革\可售交收冻结（T日） 准备数据')
        ...
        if not sql_result:
            logger().info('沪A\DVP改革\可售交收冻结（T日） 准备数据完成')
            assert True
        else:
            logger().error('沪A\DVP改革\可售交收冻结（T日） 准备数据异常')
