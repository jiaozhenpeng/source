...
class ContrastRestrictedShares(unittest.TestCase):
    ...
    沪A\DVP改革\转处置冻结（T+1日）
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['DVP']['Tday1']
    ...
    def test_restricted_shares(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\DVP改革\转处置冻结（T+1日） 对比数据')
        ...
        # 断言
        if not final_result :
            logger().info('沪A\DVP改革\转处置冻结（T+1日） 对比数据无异常')
            assert True
        else:
            logger().error('沪A\DVP改革\转处置冻结（T+1日） 对比数据异常')
