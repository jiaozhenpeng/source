...
class IndividualDividendTax(unittest.TestCase):
    ...
    def test_individualdividendtax(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A 股息红利税 对比数据')
        ...
        individualdividendtax_sql = "select * from individualdividendtax where  exchid ='0' "
        individualdividendtaxhis_sql = "select * from individualdividendtax{} where  exchid ='0' ".format(year,)
        # 数据库数据
        ...
        if not final_result:
            logger().info('沪A 股息红利税 对比数据无异常')
            assert True
        else:
            logger().error('沪A 股息红利税 对比数据异常')
