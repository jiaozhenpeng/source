...
class VirtualShareholderPurchase(unittest.TestCase):
    """
    沪A\普通买卖\股票买入虚拟股东
    """
    ...
    def test_virtual_shareholder_purchase(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\普通买卖\股票买入虚拟股东 准备数据')
        ...
        if not sql_result:
            logger().info('沪A\普通买卖\股票买入虚拟股东 准备数据完成')
            assert True
        else:
            logger().error('沪A\普通买卖\股票买入虚拟股东 准备数据异常')
