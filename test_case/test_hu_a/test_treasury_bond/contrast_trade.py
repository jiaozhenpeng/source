...
class ContrastEtfSplit(unittest.TestCase):
    ...
    沪A\国债\普通交易
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['treasury_bond']['trade']
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\国债\普通交易 对比数据')
        ...
        if not final_result:
            logger().info('沪A\国债\普通交易 对比数据无异常')
            assert True
        else:
            logger().error('沪A\国债\普通交易 对比数据异常')
