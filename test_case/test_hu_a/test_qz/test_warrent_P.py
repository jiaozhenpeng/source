...
class StockPurchase(unittest.TestCase):
    ...
    沪A\权证行权\现金选择权
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['warrent']['warrent_P']
    ...
    def test_stock_purchase(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\权证行权\现金选择权 准备数据')
        ...
        if not sql_result:
            logger().info('沪A\权证行权\现金选择权 准备数据完成')
            assert True
        else:
            logger().error('沪A\权证行权\现金选择权 准备数据异常')
