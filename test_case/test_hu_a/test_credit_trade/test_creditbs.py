...
class CresitBS(unittest.TestCase):
    """
    沪A\融资融券\担保品买卖
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['CreditTrade']['CreditBS']

    def test_credit_BS(self):
        """
        沪A\融资融券\担保品买卖
        :return:
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\融资融券\担保品买卖 准备数据')
        ...
        if not sql_result:
            logger().info('沪A\融资融券\担保品买卖 准备数据完成')
            assert True
        else:
            logger().error('沪A\融资融券\担保品买卖 准备数据异常')
