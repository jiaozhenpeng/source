...
class CSS(unittest.TestCase):
    ...
    沪A\融资融券\融券卖出
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['CreditTrade']['CSS']
    ...
    def test_CSS(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\融资融券\融券卖出 准备数据')
        ...
        if not sql_result:
            logger().info('沪A\融资融券\融券卖出 准备数据完成')
            assert True
        else:
            logger().error('沪A\融资融券\融券卖出 准备数据异常')
