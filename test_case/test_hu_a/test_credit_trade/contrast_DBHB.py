...
class ContrastEtfSplit(unittest.TestCase):
    ...
    沪A\融资融券\担保划拨
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['CreditTrade']['CreditDBHB']
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\融资融券\担保划拨 对比数据')
        ...
        # 查询sql
        stklist_sql = "select * from STKLIST where exchid='0' and stkid in('600019','600020','600021','600022') and " \
        ...
        tradinglog_sql = "select * from tradinglog{} where reckoningtime>={} and reckoningtime<={} and exchid= '0'  " \
                         "and stkid in ('600019','600020','600021','600022') and " \
        ...
        if not final_result:
            logger().info('沪A\融资融券\担保划拨 对比数据无异常')
            assert True
        else:
            logger().error('沪A\融资融券\担保划拨 对比数据异常')
