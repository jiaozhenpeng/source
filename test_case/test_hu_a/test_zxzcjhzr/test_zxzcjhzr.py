...
class StockPurchase(unittest.TestCase):
    ...
    沪A\专项资产管理计划转让\T日
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['zxzcjhzr']['Tday']
    ...
    def test_stock_purchase(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\专项资产管理计划转让\T日 准备数据')
        ...
        if not sql_result:
            logger().info('沪A\专项资产管理计划转让\T日 准备数据完成')
            assert True
        else:
            logger().error('沪A\专项资产管理计划转让\T日 准备数据异常')
