...
class Futures(unittest.TestCase):
    ...
    沪A\按值配售\T+1日（中签日）
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['IPO']['zhongqian']
    ...
    def test_futures(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\按值配售\T+1日（中签日） 准备数据')
        # 创建ipogh文件
        ...
        # if not sql_result:
        #     logger().info('沪A\按值配售\T+1日（中签日） 准备数据完成')
        #     assert True
        # else:
        #     logger().error('沪A\按值配售\T+1日（中签日） 准备数据异常')
