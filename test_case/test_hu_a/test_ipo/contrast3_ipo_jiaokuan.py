...
class ContrastEtfSplit(unittest.TestCase):
    ...
    沪A\按值配售\T+3日（资金缴款）
    """
    yaml = BaseAction().read_yaml(path=PathConfig().hu_a())['IPO']['jiaokuan']
    ...
    def test_etf_split(self):
        ...
        """
        logger().info('-------------------------------')
        logger().info('开始执行：沪A\按值配售\T+3日（资金缴款） 对比数据')
        ...
        if not final_result:
            logger().info('沪A\按值配售\T+3日（资金缴款） 对比数据无异常')
            assert True
        else:
            logger().error('沪A\按值配售\T+3日（资金缴款） 对比数据异常')
